...
export default function PokemonBox({ pokemonData, filteredPokemonData, setFilteredPokemonData, setCurrentPokemon, setShowBox } : BoxProps) {
  ...
  return (
    <div className="w-72 mb-5">
      ...
      <ul className="w-full h-60 overflow-scroll bg-[#2C2C2C] p-1 rounded-xl shadow-heavy flex flex-row items-start flex-wrap">
        {filteredPokemonData.map((pokemon, index) => {
          return (
              <li key={index}><button onClick={() => {onClickHandler(index, pokemonData, setCurrentPokemon, setShowBox)}}><img src={pokemon.sprite} className="w-16"/></button></li>
          )
        })}
      </ul>
    </div>
...
function Search({ searchTerm, setSearchTerm } : SearchProps) {
  return (
      <span className="flex flex-row items-center italic bg-[#2C2C2C] shadow-heavy rounded-xl py-1 px-2"><img className="w-3 mr-1" src={search} /><input className="w-24 outline-none rounded-xl bg-[#2C2C2C] italic" placeholder="Search" type="text" onChange={(e) => {setSearchTerm(e.target.value)}} value={searchTerm}/></span>
  )
}
...
function Sort( { setFilteredPokemonData } : SortProps) {
  ...
  return (
    <div className='bg-[#2C2C2C] rounded-xl shadow-heavy py-1 px-2 mr-2 flex flex-col'>
      ...
      {showMenu && 
        <ul>
          <li className="py-2" key="Name"><button onClick={() => { onClickHandler("Name"); setShowMenu(false) }}><p>Name</p></button></li>
